            ...
            <div className="grid grid-rows-12 w-full h-full  justify-center items-center">
                ...
                <div className=" row-span-7  w-full h-full overflow-hidden">
                    {(selectorText === "uploadImage") && selectedFile && 
                        <img className="object-contain w-full h-full max-h-full max-w-full m-2 rounded-sm" 
                        src={selectedFile}/>
                    }

                    {(selectorText === "uploadVideo") && 
                        <video key={selectedFile} className="object-contain w-full h-full max-h-full max-w-full m-2 rounded-sm" 
                            controls autoPlay>
                        ...
                        <div key={index}
                            ...
                            className=" m-1 rounded-md">
                                ...
                                className="flex justify-center items-center hover:bg-gray-300 hover:rounded-md w-28 h-28 p-1">
                                    ...
                                    {(selectorText === "uploadImage") && 
                                        <img className=" m-2 cursor-pointer  w-max h-full p-2" 
                                        src={URL.createObjectURL(file)} alt={file.name} />
                                    }
                                    {(selectorText === "uploadVideo") && 
                                        <video  className=" m-2 cursor-pointer  w-max h-full p-2" >
                                        <source src={URL.createObjectURL(file)} type="video/mp4"/>
                                        </video>
                                    }
                                    {(selectorText === "uploadDocument") && 
                                        <img className=" m-2 cursor-pointer  w-max h-full p-2" 
                                        src={URL.createObjectURL(file)} alt={file.name} />
                                    }
                                </button>
